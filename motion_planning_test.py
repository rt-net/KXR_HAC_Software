...
import get_distance_from_the_edge_test
...
distance_from_the_edge = 200 #エッジ沿いを歩行する際のエッジからの距離
...
while True:
    ...
    if angle != 0:
        distance_from_the_edge =  get_distance_from_the_edge_test.main(slope, intercept)
    else:
        distance_from_the_edge = 300
    
    finish = time.time()
    
    if distance_from_the_edge < 150:
        MOTION.stop_motion()
        MOTION.turn(angle)
        if angle > 0:
            MOTION.stop_motion()
            MOTION.walk_sideway(-(distance_from_the_edge-200))
        else:
            MOTION.stop_motion()
            MOTION.walk_sideway((distance_from_the_edge-200))
