...
class MotionLibrary:   
    ...
    def get_body_angle(self): #IMUによって機体の絶対角度を得る関数　戻り値は3要素のタプル
        try:
            yaw = self.sensor.euler[0] - self.yaw_origin #field_absolute_coordinate_yaw軸の値を読み込む　キャリブレーションの補正値が存在する場合は補正を行う
    ...
    def walk_forward(self, walk_distance): #指定距離前進
        ...
        while i < step_count:
            ...
            self.field_absolute_angle = self.get_body_angle()[0]
            ...
            print("field_absolute_coordinate_x: ", self.field_absolute_coordinate_x, "field_absolute_coordinate_y: ", self.field_absolute_coordinate_y)
    ...
    def walk_sideway(self, walk_distance): #指定距離横移動
        ...
        if walk_distance < 0: #移動量が負の時(左へ移動のとき)
            ...
            while i < step_count: #定められた歩行回数まで繰り返し
                ...
                if body_angle[0] > 0: #field_absolute_coordinate_yaw軸周りの角度が正の時
                    self.field_absolute_coordinate_x = self.field_absolute_coordinate_x + math.cos(math.radians(self.field_absolute_angle))*(-SIDE_SINGLE_STEP_TRAVEL) #自己位置に[cos(θ)*一歩の移動量 = 負]を加算
                    self.field_absolute_coordinate_y = self.field_absolute_coordinate_y + math.sin(math.radians(self.field_absolute_angle))*(SIDE_SINGLE_STEP_TRAVEL) #自己位置に[sin(θ)*一歩の移動量]を加算
                else: #field_absolute_coordinate_yaw軸周りの角度が負の時
                    self.field_absolute_coordinate_x = self.field_absolute_coordinate_x + math.cos(math.radians(self.field_absolute_angle))*(-SIDE_SINGLE_STEP_TRAVEL) #自己位置に[cos(θ)*一歩の移動量 = 負]を加算
                    self.field_absolute_coordinate_y = self.field_absolute_coordinate_y + math.sin(math.radians(self.field_absolute_angle))*(SIDE_SINGLE_STEP_TRAVEL) #自己位置に[sin(θ)*一歩の移動量 = 負]を加算
            ...
            while i < step_count: #定められた歩行回数まで繰り返し
                ...
                if body_angle[0] > 0: #field_absolute_coordinate_yaw軸周りの角度が正の時
                    self.field_absolute_coordinate_x = self.field_absolute_coordinate_x + math.cos(math.radians(self.field_absolute_angle))*(SIDE_SINGLE_STEP_TRAVEL) #自己位置に[cos(θ)*一歩の移動量]を加算
                    self.field_absolute_coordinate_y = self.field_absolute_coordinate_y + math.sin(math.radians(self.field_absolute_angle))*(-SIDE_SINGLE_STEP_TRAVEL) #自己位置に[sin(θ)*一歩の移動量 = 負]を加算
                else: #field_absolute_coordinate_yaw軸周りの角度が負の時
                    self.field_absolute_coordinate_x = self.field_absolute_coordinate_x + math.cos(math.radians(self.field_absolute_angle))*(SIDE_SINGLE_STEP_TRAVEL) #自己位置に[cos(θ)*一歩の移動量]を加算
                    self.field_absolute_coordinate_y = self.field_absolute_coordinate_y + math.sin(math.radians(self.field_absolute_angle))*(-SIDE_SINGLE_STEP_TRAVEL) #自己位置に[sin(θ)*一歩の移動量]を加算
