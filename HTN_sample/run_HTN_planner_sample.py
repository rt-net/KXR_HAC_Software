...
from planning_library_sample import PlanningLibrary

PLANNING = PlanningLibrary() #MotionPlanningLibraryのインスタンス
...
world_state.set_update_functions(WS_standing = check_standing,
                                 ...
                                 WS_in_goal = check_in_goal) #それぞれのworld_stateについて、更新用の関数をセットする
...
PT_init_pos.set_action(stand_up) #アクションの関数を指定
...
PT_walk_around.set_action(walk_in_field) #アクションの関数を指定
...
PT_face_ball.set_action(turn_to_ball) #アクションの関数を指定
...
PT_approach_ball.set_action(walk_to_ball) #アクションの関数を指定
...
PT_touch_ball.set_action(extend_arm) #アクションの関数を指定
...
PT_turn_to_goal.set_action(turn) #アクションの関数を指定
...
PT_walk_to_goal.set_action(walk_in_field) #アクションの関数を指定
...
PT_cross_goal.set_action(walk_into_goal) #アクションの関数を指定
...
while True:
    ...
    planner.make_plan([CT_root_task], world) #root_taskについて、現在のworld_stateに基づいてプランを立てる
    planner.show_plan()
